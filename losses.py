...
def bdcn_loss2(inputs, targets, l_weight=1.1):
    ...
    cost = torch.sum(cost.float().mean((1, 2, 3)))       # 这么做的意思是 在 channel width height上求平均，而不是batch(dim0)上的
    return l_weight * cost


def bdcn_loss_liu(inputs, targets, l_weight=1.1):
    # bdcn loss with the rcf approach
    targets = targets.long()
    # mask = (targets > 0.1).float()
    mask = targets.float()
...
def mse_loss(inputs, targets, l_weight=1.1):
    # bdcn loss with the rcf approach
    # inputs = torch.sigmoid(inputs)
    cost = torch.nn.MSELoss()(inputs, targets)
    return l_weight * cost


def multi_class(inputs, targets, l_weight=1.1):
    mask = targets.float()
    num_0_idx = torch.where(mask == 0)
    num_1_idx = torch.where(mask == 1)
    num_2_idx = torch.where(mask == 2)
    num_0 = float(num_0_idx[0].shape[0])  # num of value equals to 0
    num_1 = float(num_1_idx[0].shape[0])  # num of value equals to 0 and 1
    num_2 = float(num_2_idx[0].shape[0])  # num of value equals to 0 and 1 and 2

    # w1 = 1.1 * num_0 / (num_0 + num_1 + num_2)  # 0.1
    # w2 = 1.0 * num_1 / (num_0 + num_1 + num_2)  # 0.1
    # w3 = 1.0 * num_2 / (num_0 + num_1 + num_2)  # before mask[mask <= 0.1]
    # weight = torch.tensor([w1, w2, w3]).cuda()

    weights = [num_0, num_1, num_2]
    weights = [max(weights) / x for x in weights]
    weights = torch.tensor(weights).cuda()

    # loss = torch.nn.CrossEntropyLoss(weights)(inputs, targets.squeeze())
    # loss = torch.nn.CrossEntropyLoss(inputs, targets.squeeze())
