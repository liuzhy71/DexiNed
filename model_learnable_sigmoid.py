...
class CoFusion(nn.Module):
    ...
    def __init__(self, in_ch, out_ch):
        ...
        self.conv1 = nn.Conv2d(in_ch, 64, kernel_size=(3, 3), stride=(1, 1), padding=1)
        self.conv2 = nn.Conv2d(64, 64, kernel_size=(3, 3), stride=(1, 1), padding=1)
        self.conv3 = nn.Conv2d(64, out_ch, kernel_size=(3, 3), stride=(1, 1), padding=1)
        self.relu = nn.ReLU()
...
class UpConvBlock(nn.Module):
    def __init__(self, in_features, res_features, up_scale):
        super(UpConvBlock, self).__init__()
        self.up_factor = 2
        self.constant_features = 16

        layers = self.make_deconv_layers(in_features, res_features, up_scale)
        assert layers is not None, layers
        self.features = nn.Sequential(*layers)

    def make_deconv_layers(self, in_features, res_features, up_scale):
        layers = []
        all_pads = [0, 0, 1, 3, 7]
        for i in range(up_scale):
            kernel_size = 2 ** up_scale
            pad = all_pads[up_scale]  # kernel_size-1
            out_features = self.compute_out_features(i, up_scale, res_features)
    ...
    def compute_out_features(self, idx, up_scale, res_features):
        return res_features if idx == up_scale - 1 else self.constant_features
...
class DoubleConvBlock(nn.Module):
    def __init__(self, in_features, mid_features,
        ...
        self.conv1 = nn.Conv2d(in_features, mid_features, kernel_size=(3, 3), stride=(stride, stride), padding=1)
        self.bn1 = nn.BatchNorm2d(mid_features)
        self.conv2 = nn.Conv2d(mid_features, out_features, kernel_size=(3, 3), padding=1)
...
class DexiNed_learnable_sigmoid(nn.Module):
    ...
    def __init__(self, arg):
        ...
        self.dblock_3 = _DenseBlock(2, 128, 256)  # [128,256,100,100]   # cnn
        ...
        # USNet
        self.up_block_1 = UpConvBlock(in_features=64, up_scale=1, res_features=class_num)
        self.up_block_2 = UpConvBlock(in_features=128, up_scale=1, res_features=class_num)
        self.up_block_3 = UpConvBlock(in_features=256, up_scale=2, res_features=class_num)
    ...
    def forward(self, x):
        ...
        block_cat = torch.cat(results, dim=1)  # B x 6 x res_features x H x W
